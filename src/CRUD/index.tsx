...
const create = () => {
  state.people.push({ first: state.first, last: state.last, id: performance.now() })
  reset()
}
const remove = () => {
...
function usePeople() {
  ...
  return {
    people: people.filter(
      (person) => !query || person.first.startsWith(query) || person.last.startsWith(query)
    ),
...
const CRUD = () => {
  ...
  return (
    <div>
      <input name="filter" placeholder="filter..." value={snap.query} onChange={setQuery} />
      <Select />
      <label>
        <input name="last" value={snap.last} placeholder="last" onChange={setName} />
      </label>
      <label>
        <input name="first" value={snap.first} placeholder="first" onChange={setName} />
      </label>
